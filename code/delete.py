...
def run(message, bot):
    ...
    try:
        user_list = helper.read_json()  # Read the user_list from the file
        if str(chat_id) in user_list and helper.getUserHistory(chat_id) is not None:
            ...
            bot.register_next_step_handler(reply_message, process_delete_argument, bot, user_list, chat_id)
        else:
...
def process_delete_argument(message, bot, user_list, chat_id):
    """
    ...
    if text.lower() == "all":
        ...
        if date is None:
            ...
            bot.register_next_step_handler(response, handle_confirmation, bot, records_to_delete, user_list, chat_id)

def handle_confirmation(message, bot, records_to_delete, user_list, chat_id):
    """
    ...
    if message.text.lower() == "yes":
        if chat_id_str in user_list:
            # Get the user's data
            user_data = user_list.get(chat_id_str, {}).get('data', [])
            ...
            user_list[chat_id_str]['data'] = user_data
...
def deleteHistory(chat_id, user_list):
    ...
    chat_id_str = str(chat_id)
    if chat_id_str in user_list:
        user_list[chat_id_str]["data"] = []
        user_list[chat_id_str]["budget"]["overall"] = str(0)
        user_list[chat_id_str]["budget"]["category"] = {}
    return user_list